...
export default function ProductPage({ product }: any) {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        setPlaceOrderVisible(entry.isIntersecting);
      },
      {
    ...
    if (placeOrderRef.current) {
      observer.observe(placeOrderRef.current);
    }

    return () => {
      if (placeOrderRef.current) {
        observer.unobserve(placeOrderRef.current);
      }
    };
  }, []);
  ...
  return (
    <div className="relative">
      <Divider className="mb-0 relative" />
      <Container className="relative">
        <div className="pt-8">
          <Breadcrumb />
        </div>
        <ProductSingleDetails placeOrderRef={placeOrderRef} product={product} />
        <RelatedProducts
        ...
        <Subscription />
        {!isPlaceOrderVisible && (
          <div className="flex md:hidden justify-between gap-4 w-full fixed  bottom-0  z-50 pl-4 bg-white">
            <Button className="glow-button flex-1" onClick={scrollToPlaceOrder}>
